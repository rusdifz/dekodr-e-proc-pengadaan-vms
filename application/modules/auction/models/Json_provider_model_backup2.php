...
class json_provider_model extends CI_Model{
	...
	function get_chart_update($id_lelang = ''){
		...
		$get_proc = $this->db->where('id',$id_lelang)->where('del',0)->get('ms_procurement')->row_array();
		// print_r($get_proc);die;
		$type_lelang = $get_proc['auction_type'];
		
		$get_barang = $this->db->where('id_procurement',$get_proc['id'])->where('del',0)->get('ms_procurement_barang')->result_array();

		// foreach ($get_barang as $key => $value) {
		// 	$get_rank = $this->get_vendor_ranking($get_proc['id'],$value['id'],$type_lelang);
		// 	foreach ($get_rank as $key_rank => $value_rank) {
		// 		array_push($return, array('id_vendor' => $value_rank['id_peserta'], 'nama_vendor' =>$value_rank['nama_vendor']))
		// 	}
		// }
	...
	public function get_list_ranking($id_lelang,$type)
	{
		$get_barang = $this->db->where('id_procurement',$id_lelang)->where('del',0)->get('ms_procurement_barang')->result_array();
		$return = array();
		$no = 1;
		foreach ($get_barang as $key => $value) {
			$get_rank = $this->get_vendor_ranking($id_lelang,$value['id'],$type);
			foreach ($get_rank as $key_ => $value_) {
				$return[$no] = $value_['nama_vendor'];
				$no++;
			}
		}
		return $return;
